 ...
 * 图片下载工具类
 *
 * @author HeilantG {@link this#download(String, String, String)} 图片下载主方法 {@link
 *     this#biliDownload(String, String, String)} 分享图片下载
 */
@Component
public class ImgDownload {
  ...
  public String download(String imgUrl, String path, String uuid) {
    ...
    try {
      ...
      // 建立一个新的文件
      FileOutputStream fos = new FileOutputStream(new File(path + uuid + imgFormat));
  ...
  public String download(String imgUrl, String path, String uuid, String imgFormat) {
    ...
    try {
      ...
      // String b = DigestUtils.md5Hex(inputStream).toUpperCase();
      // 打开网络输入流
      DataInputStream dis = new DataInputStream(new URL(imgUrl).openStream());
      // 建立一个新的文件
      FileOutputStream fos = new FileOutputStream(new File(path + uuid + imgFormat));
      byte[] buffer1 = new byte[1024];
      int length;
