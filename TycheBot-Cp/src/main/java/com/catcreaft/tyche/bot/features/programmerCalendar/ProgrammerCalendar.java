...
public class ProgrammerCalendar {
  ...
  public static String getCalendar(int... key) {
    ...
    str.append(
        "座位朝向：面向"
            + hl.directions[hl.random(hl.getIday(), 2) % hl.getDirections().length]
            + "写程序，BUG 最少。");
    str.append("\n今日宜饮：" + hl.pickRandomDrinks(1));
  ...
  public StringBuffer pickTodaysLuck() {
    ...
    for (int i = 0; i < numGood; i++) {
      String s =
          "\n "
              + eventArr.get(i).get("name")
              + (!"".equals(eventArr.get(i).get("good")) ? ":" + eventArr.get(i).get("good") : "");
      str.append(s);
    }
    str.append("\n不宜：");
    for (int i = 0; i < numBad; i++) {
      str.append(
          "\n "
              + eventArr.get(numGood + i).get("name")
              + (!"".equals(eventArr.get(numGood + i).get("bad"))
                  ? ":" + eventArr.get(numGood + i).get("bad")
                  : ""));
    }
    return str;
  }
  ...
  // 枚举 中随机挑选 size 个
  public List<Map<String, String>> pickRandomActivity(
      List<ActivitiesEnum> _activities, Integer size) {
  ...
  public Map<String, String> parse(ActivitiesEnum ae) {
    ...
    if (map.get("name").indexOf("%v") != -1) {
      map.put(
          "name", map.get("name").replaceAll("%v", varNames[random(iday, 12) % varNames.length]));
    }
